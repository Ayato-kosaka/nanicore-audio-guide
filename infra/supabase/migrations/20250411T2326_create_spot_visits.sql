...
CREATE POLICY "Authenticated users can get their own spot_visits"
    ON spot_visits
    FOR SELECT
    TO authenticated
...
CREATE POLICY "Authenticated users can insert their own spot_visits"
    ON spot_visits
    FOR INSERT
    TO authenticated
...
CREATE POLICY "Authenticated users can update their own spot_visits"
    ON spot_visits
    FOR UPDATE
    TO authenticated
    USING (auth.uid() = user_id)
