...
import { FindOrCreatePlaceFromImageResponse } from "../../../../shared/api/findOrCreatePlaceFromImage.schema";
import { convertPrismaToSupabase_ExtPlaces } from "../../../../shared/converters/convert_ext_places";
import { findOrCreatePlaceFromImageUri } from "../services/findOrCreatePlaceFromImageUri";
 ...
 * @returns FindOrCreatePlaceFromImageResponse JSON
 */
export const findOrCreatePlaceFromImage = withValidatedAuthHandler(
	z.object({}),
	async function findOrCreatePlaceFromImage({ req, res, requestId, userId, functionName, file }) {
		if (!file) return;
		...
		const extPlace = await findOrCreatePlaceFromImageUri(imageUri, requestId, userId);

		const response: FindOrCreatePlaceFromImageResponse = {
