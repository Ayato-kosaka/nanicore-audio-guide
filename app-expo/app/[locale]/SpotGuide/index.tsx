...
} from "@shared/api/listRecommendedSpotsByVisitHistory.schema";
...
export default function SpotGuideScreen() {
	...
	const { extSpots, imageUri, takenPhotoStoragePath }: SpotGuideParams = useMemo(
		() => deserializeSpotGuideParams(serializedParams),
		[serializedParams],
	);
	...
	useEffect(() => {
		...
		withLoading(async () => {
			...
			try {
				...
				const guides = await callCloudFunction<ListSpotGuidesRequest, ListSpotGuidesResponse>(
					"listSpotGuides",
					{ spotId: extSpots.id, languageTag: locale },
					"v1",
				);
				setSpotGuides(
					guides.spotGuides.map((spotGuide) => ({
			...
			<View style={styles.buttonContainer}>
				...
				<View style={styles.button}>
					{currentIndex < carouselItems.length - 1 &&
						renderControlButton("chevron-right", () => carouselRef.current?.next(), "right-button")}
				</View>
				<View style={styles.button}>
