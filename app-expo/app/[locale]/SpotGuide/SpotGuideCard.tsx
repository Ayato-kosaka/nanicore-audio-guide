...
const SpotGuideCard = ({
			...
			try {
				...
				newSound.setOnPlaybackStatusUpdate((status) => {
					...
					if (status.didJustFinish) {
						...
						// ガイド再生終了時に訪問記録を更新
						if (user?.id && guide?.id) {
							supabase.from("spot_visits").update({ represent_guide_id: guide.id }).eq("id", visitIdRef.current);
						}
					}
				});
	...
	const generateSpotGuide = useCallback(async () => {
		try {
			const { spotGuide, audioUrl } = await callCloudFunction<GenerateSpotGuideRequest, GenerateSpotGuideResponse>(
				"generateSpotGuide",
				{ extSpot: convertPrismaToSupabase_ExtSpots(spot), languageTag: locale },
				"v1",
			);

			const newGuide = { ...convertSupabaseToPrisma_SpotGuides(spotGuide), audioUrl };
			setSpotGuideList((prev) => [...prev, newGuide]);
